...
async def create_staff_attendance_route(
    staff_attendance: StaffAttendanceCreate, db: AsyncSession = Depends(get_db)
):
...
async def get_staff_attendance_route(
    staff_attendance_id: int, db: AsyncSession = Depends(get_db)
):
...
async def update_staff_attendance_route(
    ...
    db_staff_attendance = await update_staff_attendance(
        ...
        staff_attendance_update=staff_attendance_update,
    )
    if db_staff_attendance is None:
        raise HTTPException(status_code=404, detail="Staff Attendance not found")
    return db_staff_attendance
...
async def delete_staff_attendance_route(
    staff_attendance_id: int, db: AsyncSession = Depends(get_db)
):
...
async def get_all_staff_attendance_route(
    company_id: UUID,
    skip: int = 0,
    limit: int = 100,
    db: AsyncSession = Depends(get_db),
):
    return await get_all_staff_attendance(
        db=db, company_id=company_id, skip=skip, limit=limit
    )


@router.get("/by_date_range/", response_model=List[StaffAttendanceResponse])
async def get_staff_attendance_by_date_range_route(
    company_id: UUID,
    start_date: datetime,
    end_date: datetime,
    db: AsyncSession = Depends(get_db),
):
    return await get_staff_attendance_by_date_range(
